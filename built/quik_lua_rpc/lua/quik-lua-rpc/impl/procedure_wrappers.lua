...
module["datasource.SetUpdateCallback"] = function (args)
  ...
  return requireNonNil(ds:SetUpdateCallback(
      function (index)
            local props = {uuid = ds_uuid, index = index}
            props.O = ds:O(index)
            props.H = ds:H(index)
            props.L = ds:L(index)
            props.C = ds:C(index)
            props.V = ds:V(index)
            props.T = ds:T(index)
            props.Size = ds:Size(index)
            _G.OnDataSourceUpdate(props)
      end
  ))
end
...
module["sendTransaction"] = function (args) 
  ...
  err = requireNonNil(_G.sendTransaction(args.transaction))

  return utils.Cp1251ToUtf8(err)
end
