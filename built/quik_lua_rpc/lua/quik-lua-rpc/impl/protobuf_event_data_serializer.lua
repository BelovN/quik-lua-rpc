...
function ProtobufEventDataSerializer:OnDataSourceUpdate (update_info)
  ...
  if update_info.T then
    pb_update_info.time = to_pb_obj(qlua_pb_types.qlua_structures.DataSourceTime, update_info.T)
  end
  ...
  if update_info.O then
    pb_update_info.open = tostring(update_info.O)
  end

  if update_info.H then
    pb_update_info.high = tostring(update_info.H)
  end

  if update_info.L then
    pb_update_info.low = tostring(update_info.L)
  end

  if update_info.C then
    pb_update_info.close = tostring(update_info.C)
  end

  if update_info.V then
    pb_update_info.volume = tostring(update_info.V)
  end
